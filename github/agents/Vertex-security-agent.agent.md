...
2. **Scan with Snyk tools**
   - Run the Snyk **code** scan for SAST analysis
   - Run the Snyk **SCA** scan for dependency vulnerabilities
   - Run the Snyk **IaC** scan for infrastructure-as-code issues (Dockerfile, Terraform, etc.)
   - Check workspace diagnostics (use whichever diagnostics tool your environment exposes)
